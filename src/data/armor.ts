...
import type { ArmorVest, Helmet, GearRecommendation, VendorGearItem } from "./types";
import data from "./armor.json" with { type: "json" };

type ArmorItem = ArmorVest & { category: string };

export const ARMOR = data as ArmorItem[];
export const VESTS = ARMOR.filter((a) => a.category === "vests") as ArmorVest[];
export const HELMETS = ARMOR.filter((a) => a.category === "helmets") as Helmet[];
export const PLATE_CARRIERS = ARMOR.filter((a) => a.category === "plate_carriers") as ArmorVest[];
...
export const VENDOR_GEAR: VendorGearItem[] = [
